...
def test_breadth_trav_right_tree(init_bst_right):
    """."""
    gen = init_bst_right.breadth_first()
    assert next(gen) == 5
    assert next(gen) == 10
    assert next(gen) == 15
    assert next(gen) == 20
...
def test_breadth_trav_left_tree(init_bst_left):
    """."""
    gen = init_bst_left.breadth_first()
    assert next(gen) == 20
    assert next(gen) == 15
    assert next(gen) == 10
    assert next(gen) == 5
...
def test_breadth_trav_list(init_bst_list):
    """."""
    gen = init_bst_list.breadth_first()
    assert next(gen) == 6
    assert next(gen) == 3
    assert next(gen) == 8
    assert next(gen) == 1
